...
public class Player
{
    ...
    private unsafe void OnLoad()
    {
        ...
        if (settings.ffRender)
        {
            ...
            ffmpeg.Start();
            Wrapper = new(ffmpeg, settings.Width, settings.Height, 4);
    ...
    unsafe void OnRenderFrame(double args)
    {
        ...
        if (settings.ffRender && midiTime < StopPoint)
        {
            ...
            int widthBytes = (int)settings.Width * 4;
            ...
            {
                int rowPitchInt = (int)rowPitch;
        
                Parallel.For(0, height, y =>
                {
                    int srcY = flipY ? heightMinusOne - y : y;
                    nint srcOffset = srcY * rowPitchInt;
                    nint dstOffset = y * stride;
    ...
    private void DrawPerformanceOverlay()
    {
        ...
        ImGui.Image(_controller.GetOrCreateImGuiBinding(GD.ResourceFactory, text.textureV), new(ImGui.GetWindowWidth() - ImGui.GetCursorPosX(), ImGui.GetWindowHeight() - ImGui.GetCursorPosY()));
        ImGui.End();
    }
    ...
    void OnUpdateFrame(double args, InputSnapshot snap)
    {
        ...
        {
            file.ParseUpTo(midiTime, midiTime + deltaTimeOnScreen + (tempoFrameStep * 20 * mv));
        }
